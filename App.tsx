...
const App: React.FC = () => {
  ...
  const FloralSideDecoration = ({ side, top }: { side: 'left' | 'right', top: string }) => {
    const isLeft = side === 'left';
    return (
      <div 
        className={`absolute z-20 w-32 h-32 md:w-40 md:h-40 pointer-events-none select-none ${isLeft ? '-left-6' : '-right-6'} transition-all`}
        style={{ top }}
      >
        <img 
          src={getDriveMediaUrl(weddingData.assets.floralSide || weddingData.assets.floralCorner)} 
          alt="floral-side" 
          className={`w-full h-full object-contain opacity-80 drop-shadow-lg ${isLeft ? 'rotate-[90deg]' : '-rotate-[90deg] scale-x-[-1]'} animate-floral-sway-gentle`}
        />
      </div>
    );
  };
    ...
    <MobileWrapper>
      <div className="font-sans pb-24">
        ...
        <section className="py-20 px-6 text-center relative overflow-hidden" style={getSectionStyle(weddingData.assets.coupleBg)}>
          <FloralSideDecoration side="left" top="10%" />
          <FloralSideDecoration side="right" top="60%" />
        ...
        <section className="py-20 px-4 relative bg-white/30 backdrop-blur-sm overflow-hidden" style={getSectionStyle(weddingData.assets.eventsBg)}>
          <FloralSideDecoration side="right" top="20%" />
          <FloralSideDecoration side="left" top="70%" />
        ...
        <section className="py-20 px-4 relative" style={getSectionStyle(weddingData.assets.galleryBg)}>
          <FloralSideDecoration side="left" top="5%" />
        ...
        <section ref={giftSectionRef} className="py-20 px-6 relative bg-blue-50/50" style={getSectionStyle(weddingData.assets.giftBg)}>
          <FloralSideDecoration side="right" top="15%" />
